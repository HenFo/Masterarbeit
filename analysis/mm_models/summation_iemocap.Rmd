---
title: An R Markdown document converted from "summation_iemocap.ipynb"
...
BASE_PATH = os.path.join("results", "early_sum")
...
def get_result_dataframe(dataset: str) -> pd.DataFrame:
    ...
    normal_stage2 = os.path.join(BASE_PATH, dataset, "preds_test_normal_stage_2.json")

    normal_df = utils.build_result_dataframe(normal_path)
    audio_df = utils.build_result_dataframe(audio_path)
    stage2_df = utils.build_result_dataframe(normal_stage2)
    ierc_df = utils.get_instructerc_results(INSTRUCTERC_BASE_PATH, dataset)

    assert len(normal_df) == len(audio_df) == len(ierc_df) == len(stage2_df)

    results = utils.merge_result_dataframes([normal_df, audio_df, ierc_df, stage2_df], ["normal", "audio", "ierc", "normal_stage_2"])
...
print(f"F1-Score des Summation Modells: {100*summation_model_f1:.2f}")
print(f"F1-Score mit nur audio input: {100*audio_only_f1:.2f}")
print(f"F1-Score des Basismodells: {100*text_only_f1:.2f}")
print(f"Änderung: {(100*summation_model_f1 - 100*text_only_f1):.2f}")
...
```{python}
utils.print_confusion_matrix(df, target_labels=labels[::-1], output_column="output", name = "images/summation_iemocap_combined_cm.png", title="Confusion Matrix on IEMOCAP")
```
...
```{python}
utils.print_confusion_matrix_difference(df, target_labels=labels[::-1], output_column1="output", output_column2="output_ierc", name = "images/summation_iemocap_diff_cm.png", title="Confusion Matrix difference on IEMOCAP")
```

```{python}
df[combined_model & not_audio_only_model & not_text_only_model]["target"].value_counts()
```
...
p.show()
p.save("images/summation_iemocap_proportions.png", width=10, height=5)
```

```{python}
df[combined_model & audio_only_model]["target"].value_counts()
```
...
```{python}
f1_score(df["target"],df["output_normal_stage_2"], labels=labels, average="weighted")
```
...
x <- list(Audio = py$df[py$audio_only_model,], Combined = py$df[py$combined_model,], Text = py$df[py$text_only_model,])
x <- lapply(x, rownames)
